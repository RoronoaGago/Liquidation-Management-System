...
def generate_liquidation_report_data(
    ...
    if school_ids:
        school_id_list = school_ids.split(',') if isinstance(school_ids, str) else school_ids
        queryset = queryset.filter(
    ...
    for liquidation in queryset:
        ...
        report_data.append({
            ...
            'reviewed_by_district': liquidation.reviewed_by_district,
            'reviewed_by_liquidator': liquidation.reviewed_by_liquidator,
            'reviewed_by_division': liquidation.reviewed_by_division,
        })
...
def generate_liquidation_csv_report(report_data, filters):
    ...
    if filters.get('start_date') and filters.get('end_date'):
        filename_parts.append(f"{filters['start_date']}_to_{filters['end_date']}")
    ...
    if filters.get('start_date') and filters.get('end_date'):
        writer.writerow([f'Date Range: {filters["start_date"]} to {filters["end_date"]}'])
    ...
    for item in report_data:
        writer.writerow([
            ...
            item['refund'] or '',
            f"{item['reviewed_by_district'].first_name} {item['reviewed_by_district'].last_name}" if item['reviewed_by_district'] else '',
            f"{item['reviewed_by_liquidator'].first_name} {item['reviewed_by_liquidator'].last_name}" if item['reviewed_by_liquidator'] else '',
            f"{item['reviewed_by_division'].first_name} {item['reviewed_by_division'].last_name}" if item['reviewed_by_division'] else '',
        ])
...
def generate_liquidation_excel_report(report_data, filters):
    ...
    for row_num, item in enumerate(report_data, 11):
        ...
        ws.cell(row=row_num, column=10, value=item['created_at'].date() if item['created_at'] else '')
        ws.cell(row=row_num, column=11, value=item['date_submitted'].date() if item['date_submitted'] else '')
        ws.cell(row=row_num, column=12, value=item['date_liquidated'].date() if item['date_liquidated'] else '')
        ws.cell(row=row_num, column=13, value=item['refund'] or '')
        ws.cell(row=row_num, column=14, value=f"{item['reviewed_by_district'].first_name} {item['reviewed_by_district'].last_name}" if item['reviewed_by_district'] else '')
        ws.cell(row=row_num, column=15, value=f"{item['reviewed_by_liquidator'].first_name} {item['reviewed_by_liquidator'].last_name}" if item['reviewed_by_liquidator'] else '')
        ws.cell(row=row_num, column=16, value=f"{item['reviewed_by_division'].first_name} {item['reviewed_by_division'].last_name}" if item['reviewed_by_division'] else '')
